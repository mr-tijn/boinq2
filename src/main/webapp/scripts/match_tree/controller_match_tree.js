boinqApp.controller("MatchTreeController",['$scope','queryFromMatch',function($scope,queryFromMatch) {
	...
	$scope.submit = function(tree) {
		...
		queryFromMatch($scope.rootNode).then(function(result) {$scope.generatedQuery = result.data;},function (error) {$scope.errorMessage = error;});
	};
